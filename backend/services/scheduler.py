...
Cô xin gửi lại nội dung buổi học ngày hôm nay của các con ạ:
⭐ Môn Tiếng Anh:
...
Thứ Năm (26/03): kiểm tra môn Toán Unit 11&12. Cô xin gửi tài liệu ôn tập của các con ạ:
https://drive.google.com/drive/folders/1gWGLhZTgxiHZ2fTgopsz9smlJIiZetXU?usp=drive_link
Thứ Sáu (27/03): kiểm tra môn Tiếng Anh Unit 7. Cô xin gửi trước các tài liệu ôn tập của các con ạ:
...
Cô xin thông báo lịch kiểm tra sắp tới của các con như sau:
Thứ Năm (26/03): kiểm tra môn Toán Unit 11&12. Cô xin gửi tài liệu ôn tập cho các con:
https://drive.google.com/drive/folders/1gWGLhZTgxiHZ2fTgopsz9smlJIiZetXU?usp=drive_link
...
async def trigger_daily_summary_demo(
    ...
    """
    Send the hardcoded daily summary to the requested channels.

    Args:
        content:  Ignored (kept for backward compatibility).
    ...
    # Google Chat — post student-facing summary via Chat API
    if send_gchat:
        try:
            from services.chat.google_chat_listener import get_google_chat_listener

            listener = get_google_chat_listener()
            space_id = settings.GOOGLE_CHAT_SPACE_ID
            if space_id:
                await listener._reply_to_chat(space_id, DEMO_LESSON_CONTENT_STUDENTS)
                logger.info("[SCHEDULER] Daily summary sent to Google Chat via Chat API")
            else:
                # Fallback to notification service (webhook)
                ...
                notification = Notification(
                    ...
                    message=DEMO_LESSON_CONTENT_STUDENTS,
                )

                svc = NotificationService()
                results = await svc.send(notification)
                if results and results[0].success:
                    logger.info("[SCHEDULER] Daily summary sent to Google Chat via webhook")
                else:
                    err = results[0].error_message if results else "no notifier enabled"
                    logger.warning(
                        f"[SCHEDULER] Google Chat send failed: {err}"
                    )
        except Exception as e:
            logger.error(f"[SCHEDULER] Google Chat daily summary error: {e}")

    # Zalo — post parent-facing summary via ZaloNotifier (in-memory store → REST polling)
    if send_zalo:
        try:
            ...
            notification = ZNotification(
                ...
                title=f"Daily Summary — {date_str}",
                message=DEMO_LESSON_CONTENT_PARENTS,
            )
